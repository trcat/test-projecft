    ...
    <div id="container"  v-loading="containerLoading">
        ...
        <el-card v-for="item in papers" :key="item.id">
            <div slot="header">
                ...
                <el-button class="card-button" type="text" @click="test(item)" :disabled="getTestStatus(item.start_time).disabled" circle>开始考试 >></el-button>
            </div>
...
export default {
    ...
    methods: {
        ...
        getTestStatus(time) {
            ...
            const disabled = Date.parse(date) - Date.now() > 0;
            
            return {disabled, dateString};
        }
    },
    mounted() {
        const callback = (r) => {
